...
The current setup does have one safeguard: the CD pipeline takes a database backup before every deploy, so if a bad release goes out there is always a restore point.
